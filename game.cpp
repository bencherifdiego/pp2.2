...
#define REF_PERFORMANCE 68028.1 //UPDATE THIS WITH YOUR REFERENCE PERFORMANCE (see console after 2k frames)
//1. 73466
//2. 68028.1
...
void Game::Init()
{
    ...
    {
        Tank tank = Tank(start_blue_x + ((i % max_rows) * spacing), start_blue_y + ((i / max_rows) * spacing), BLUE, &tank_blue, &smoke, 1200, 600, tank_radius, TANK_MAX_HEALTH, TANK_MAX_SPEED, nullptr);
    ...
    {
        Tank tank = Tank(start_red_x + ((i % max_rows) * spacing), start_red_y + ((i / max_rows) * spacing), RED, &tank_red, &smoke, 80, 80, tank_radius, TANK_MAX_HEALTH, TANK_MAX_SPEED, nullptr);
        tanks.push_back(tank);
        sorted_tanks.push_back(&tanks.back());
        grid.add(&tanks.back());
    }
...
int Game::partition(vector<Tank>& original, int left, int right)
{
...
void Game::quicksort(vector<Tank>& original, int left, int right)
{
    if (left < right)
    {
